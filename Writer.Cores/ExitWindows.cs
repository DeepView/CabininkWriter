...
namespace Cabinink.Writer.Cores
{
   ...
   public sealed class ExitWindows
   {
      ...
      /// </summary>
      /// <param name="_flags">指定格式化处理和如何翻译 lpSource 参数. dwFlags的低字节指定函数如何处理输出缓冲区的换行. 低字节也可以指定格式化后的输出缓冲区的最大宽度.</param>
      /// <param name="_src">指定消息定义的位置. 此参数的类型依据 dwFlags 参数的设定.</param>
      /// <param name="_msgid">指定消息的消息标志ID. 如果 dwFlags 参数包含 FORMAT_MESSAGE_FROM_STRING ，则该参数被忽略.</param>
      /// <param name="_langid">指定消息的语言ID. 如果 dwFlags 参数包含 FORMAT_MESSAGE_FROM_STRING，则该参数被忽略.</param>
      /// <param name="_buffer">用来放置格式化消息(以null结束)的缓冲区.如果 dwFlags 参数包括 FORMAT_MESSAGE_ALLOCATE_BUFFER, 本函数将使用LocalAlloc函数定位一个缓冲区，然后将缓冲区指针放到 lpBuffer 指向的地址.</param>
      /// <param name="_nsize">如果没有设置 FORMAT_MESSAGE_ALLOCATE_BUFFER 标志, 此参数指定了输出缓冲区可以容纳的TCHARs最大个数. 如果设置了 FORMAT_MESSAGE_ALLOCATE_BUFFER 标志，则此参数指定了输出缓冲区可以容纳的TCHARs 的最小个数. 对于ANSI文本, 容量为bytes的个数; 对于Unicode 文本, 容量为字符的个数.</param>
      /// <param name="_arguments">数组指针,用于在格式化消息中插入信息. 格式字符串中的 A %1 指示参数数组中的第一值; a %2 表示第二个值; 以此类推.</param>
      ...
      private extern static int FormatMessage(int _flags, IntPtr _src, int _msgid, int _langid, StringBuilder _buffer, int _nsize, int _arguments);
      /// <summary>
      /// 将指定的可执行模块映射到调用进程的地址空间
      /// </summary>
      /// <param name="_libfname">可执行模块(dll or exe)的名字：以null结束的字符串指针. 该名称是可执行模块的文件名，与模块本身存储的,用关键字LIBRARY在模块定义文件(.def)中指定的名称无关,</param>
      ...
      private extern static IntPtr LoadLibrary(string _libfname);
      /// <summary>
      /// 将装载的dll引用计数器减一，当引用计数器的值为0后，模块将从调用进程的地址空间退出，模块的句柄将不可再用
      /// </summary>
      /// <param name="_libmodule">dll模块的句柄. LoadLibrary 或者 GetModuleHandle 函数返回该句柄</param>
      ...
      private extern static int FreeLibrary(IntPtr _libmodule);
      /// <summary>
      /// 获取外部函数的入口地址
      /// </summary>
      /// <param name="_module">Dll的句柄，包含了函数或者变量，LoadLibrary 或 GetModuleHandle 函数返回该句柄 </param>
      /// <param name="_procname">以null结束的字符串指针，包含函数或者变量名，或者函数的顺序值，如果该参数是一个顺序值，它必须是低序字be in the low-order word,高序字(the high-order)必须为0</param>
      /// <returns>如果执行成功, 返回值为外部函数或变量的地址。如果执行失败，返回值为NULL。如果要获取更多错误信息，请调用Marshal.GetLastWin32Error.</br></returns>
      [DllImport("kernel32.dll", EntryPoint = "GetProcAddress", CharSet = CharSet.Ansi)]
      private extern static IntPtr GetProcAddress(IntPtr _module, string _procname);
      /// <summary>
      ...
      /// 获取更高级别的操作系统权限
      /// </summary>
      /// <param name="_privilege">需要获得的权限。</param>
      ...
      /// </summary>
      /// <param name="_library">包含指定方法的库文件。</param>
      /// <param name="_method">指定方法的入口。</param>
      /// <returns>如果存在指定方法，返回True，否则返回False。</returns>
      public static bool CheckEntryPoint(string _library, string _method)
      {
         IntPtr libPtr = LoadLibrary(_library);
         if (!libPtr.Equals(IntPtr.Zero))
         {
            if (!GetProcAddress(libPtr, _method).Equals(IntPtr.Zero))
            {
               FreeLibrary(libPtr);
               return true;
            }
      ...
      /// 将错误号转换为错误消息
      /// </summary>
      /// <param name="_errno">需要转换的错误号。</param>
      /// <returns>代表指定错误号的字符串。</returns>
      public static string FormatError(int _errno)
      {
         StringBuilder buffer = new StringBuilder(255);
         FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, IntPtr.Zero, _errno, 0, buffer, buffer.Capacity, 0);
