...
namespace Cabinink.Writer.UI
{
   public partial class frmMainInterface : Cabinink.Writer.UI.VsSkinFormBase
   {
      ...
      public TreeNode GetRootNode(TreeNode _selected)
      {
         ...
         string fullpath = _selected.FullPath;  //The exception be throwed.
      ...
      {
         tmrSaveChapter.Enabled = true;
         if (EditingProj != null) tmrSaveChapter_Tick(sender, e);
         trvNovelTree.Refresh();
         TreeNode node_root;
         TreeNode node_selected = trvNovelTree.SelectedNode;
         if (node_selected != null)
         {
            node_root = GetRootNode(trvNovelTree.SelectedNode);
         }
         else
         {
            throw new NullReferenceException("函数frmMainInterface.GetRootNode(TreeNode)参数没有设置对象引用！");
         }
         GetProjectAccessSelectedNode().ReadAll();
         EditingProj = app.Projects[GetRootNode(node_root).Index];
         SelectedNode = trvNovelTree.SelectedNode;
         #region LoadChapterToEditor
         if (GetNodeDeepin(node_root.FullPath) == 2)
         {
            Editing = EditingProj.CurrentNovel[node_root.Parent.Index][node_root.Index];
            rtbBody.Text = Editing.Body;
         }
         //MessageBox.Show(EditingProj.ChapterExists(1, 1).ToString());
      ...
      private void frmMainInterface_FormClosing(object sender, FormClosingEventArgs e)
      {
         ...
         if (EditingProj != null) EditingProj.SaveAll();
         e.Cancel = true;
         Visible = false;
      }
      ...
      {
         frmNtpClient synctime = new frmNtpClient();
         synctime.ShowDialog(this);
      }
   }
}
