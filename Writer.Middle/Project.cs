...
{
   /// <summary>
   /// 用于确定项目的创建源的应用程序信息结构体
   /// </summary>
   [StructLayout(LayoutKind.Sequential, Pack = 1)]
   [Obsolete("This temporary structure in the current version as a function of a reserved nature.")]
   [DebuggerDisplay("AssemblyName = {AssemblyName}, ProductVersion = {ProductVersion}")]
   public struct SAssemblyInformation
   {
      /// <summary>
      /// 当前应用程序的程序集名称，这个名称一般而言是针对于用户的
      /// </summary>
      public string AssemblyName;
      /// <summary>
      /// 当前应用程序的完全限定名称，这是针对于开发者的信息
      /// </summary>
      public string AssemblyFullName;
      /// <summary>
      /// 当前计算机的名称，比如说：“Cabinink-TamanyuSmoke”，而非“\\Cabinink-TamanyuSmoke”
      /// </summary>
      public string ComputerName;
      /// <summary>
      /// 使用当前应用程序的Windows用户，这个字段的格式为：&lt;USER_NAME&gt;
      /// </summary>
      public string CurrentWindowsUser;
      /// <summary>
      /// 当前应用程序的产品版本字符串，而非文件版本字符串，比如说：1.0.1607.56
      /// </summary>
      public string ProductVersion;
      /// <summary>
      /// 构造函数，创建一个指定计算机名称的程序集信息结构实例
      /// </summary>
      /// <param name="_reserved">保留关键字，一般无实际意义，可以取System.Int32数据类型合法数值范围内的任意值。</param>
      public SAssemblyInformation(int _reserved = 0)
      {
         AssemblyName = AssemblyInformation.ApplicationName;
         AssemblyFullName = AssemblyInformation.AssemblyFullName;
         ComputerName = EnvironmentInformation.GetComputerName();
         CurrentWindowsUser = Environment.UserName;
         ProductVersion = AssemblyInformation.ProductionVersion;
      }
      /// <summary>
      /// 构造函数，创建一个指定程序集名称、完全限定名称、计算机名称、当前Windows用户名称和产品版本号字符串的程序集信息结构实例
      /// </summary>
      /// <param name="_asmname">指定的程序集名称。</param>
      /// <param name="_fullname">指定的程序集完全限定名称。</param>
      /// <param name="_compname">指定的计算机名称。</param>
      /// <param name="_sysusr">当前登录并使用该应用程序的Windows用户名称。</param>
      /// <param name="_prdtver">当前程序集的产品版本号字符串。</param>
      public SAssemblyInformation(string _asmname, string _fullname, string _compname, string _sysusr, string _prdtver)
      {
         AssemblyName = _asmname;
         AssemblyFullName = _fullname;
         ComputerName = _compname;
         CurrentWindowsUser = _sysusr;
         ProductVersion = _prdtver;
      }
      /// <summary>
      /// 将产品版本字符串转换为一个有效的Version实例
      /// </summary>
      /// <returns>成功获取该Version实例之后，则将这个Version实例返回。</returns>
      public Version GetProductVersionExample()
      {
         return new Version(ProductVersion);
      }
      /// <summary>
      /// 检查创建该实例对应的作品实例是否符合当前应用程序的版本兼容性条件
      /// </summary>
      /// <returns>如果创建该实例对应的作品实例符合当前应用程序的版本兼容性条件，则将返回true，否则将会返回false。</returns>
      public bool IsBackwardCompatibledWithVersion()
      {
         return GetProductVersionExample() > new Version(AssemblyInformation.ProductionVersion) ? false : true;
      }
      /// <summary>
      /// 检查创建该实例对应的作品实例是否符合当前应用程序的作品项目数据库结构兼容性条件
      /// </summary>
      /// <returns>如果创建该实例对应的作品实例符合当前应用程序的作品项目数据库结构兼容性条件，则将返回true，否则将会返回false。</returns>
      [Obsolete("在当前版本中该函数暂时没有符合预期判定条件的代码。")]
      public bool IsBackwardCompatibledWithProjectDatabase()
      {
         return true;
      }
   }
   ...
   public class Project
   {
      ...
      /// 当前项目的程序集信息
      /// </summary>
      private SAssemblyInformation asminfo;
      /// <summary>
      /// 当前项目所包含的作品
      /// </summary>
      private Novel novel;
      ...
      /// 获取或设置当前实例在创建时的程序集信息
      /// </summary>
      public SAssemblyInformation AssemblyInformation
      {
         get { return asminfo; }
         set { asminfo = value; }
      }
      ...
      public void InitializeProject()
      {
         ...
         if (Database.DataTableExists("appinfo") == false)
            Database.InvokeSql(ReadSqlDocument(@"databases\crtt_appinfo_sql.document-ci"));
         Database.Disconnect();
      }
      /// <summary>
      /// 从数据库中读取创建项目时保存的程序集信息
      /// </summary>
      /// <returns>如果该操作成功则返回true，否则将会返回false。</returns>
      public bool ReadAssemblyInformation()
      {
         bool iscomp = true;
         SQLiteDataReader reader;
         SAssemblyInformation appinfo = new SAssemblyInformation(0);
         Database.InitializeConnection();
         Database.Connect();
         reader = Database.InvokeSqlToReader(@"select * from appinfo");
         if (reader.HasRows)
         {
            while (reader.Read())
            {
               appinfo.AssemblyName = reader.GetString(reader.GetOrdinal("_appname"));
               appinfo.AssemblyFullName = reader.GetString(reader.GetOrdinal("_fullname"));
               appinfo.ComputerName = reader.GetString(reader.GetOrdinal("_compname"));
               appinfo.CurrentWindowsUser = reader.GetString(reader.GetOrdinal("_sysusr"));
               appinfo.ProductVersion = reader.GetString(reader.GetOrdinal("_prdtver"));
            }
         }
         else iscomp = false;
         AssemblyInformation = appinfo;
         reader.Close();
         Database.Disconnect();
         return iscomp;
      }
      ...
      /// 保存程序集信息到数据库
      /// </summary>
      /// <returns>如果该操作成功则返回true，否则返回false。</returns>
      public bool SaveAssemblyInformation()
      {
         int isins = 0;
         SAssemblyInformation ai = AssemblyInformation;
         Database.InitializeConnection();
         Database.Connect();
         string s_asmname = "(\x27" + ai.AssemblyName + "\x27\x2c";
         string s_fullname = "\x27" + ai.AssemblyFullName + "\x27\x2c";
         string s_compname = "\x27" + ai.ComputerName + "\x27\x2c";
         string s_sysusr = "\x27" + ai.CurrentWindowsUser + "\x27\x2c";
         string s_prdtver = "\x27" + ai.ProductVersion + "\x27)";
         string sql_values = s_asmname + s_fullname + s_compname + s_sysusr + s_prdtver;
         Database.InvokeSql(@"delete from appinfo");
         isins = Database.InvokeSql(@"insert into appinfo values" + sql_values);
         Database.Disconnect();
